...
- Payment schedule: payable `financial_obligations` rows with `fob_source_type = purchase_order` and `fob_source_id` = the PO id. Amounts are StockIt integer minor units; the app does not convert line-price majors to minors. Each installment carries an optional designation code, picked from StockIt's `designation_codes` and shown as `CODE - Name`.
- Print: browser print render of the PO (supplier block, lines, totals, payment schedule). The header carries one line naming who stands behind the order rather than the internal status: `Issued by alex. Approved by admin. Payment: unpaid.` "Issued by" is whoever moved the order to `issued`, falling back to whoever drafted it; "Approved by" appears only once someone has actually approved, so a draft or rejected order never claims otherwise. Use the browser's Save as PDF.
