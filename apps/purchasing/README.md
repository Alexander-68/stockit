...
- Status, approvals & payment: the full lifecycle (`draft`, `pending_approval`, `approved`, `rejected`, `issued`, `confirmed`, `partially_received`, `received`, `invoiced`, `closed`, `on_hold`, `pending_revision`, `cancelled`) plus an independent payment tag (`unpaid`, `partially_paid`, `paid`, `refunded`). "Apply status change" posts status, payment tag and note to StockIt; The submit button (draft orders only) reads "Submit for approval" when an active `purchase_order` approval rule matches the order amount, and "Approve" when none does — because StockIt then approves the order outright instead of routing it. It shows the resulting steps, with Approve/Reject offered on the first pending step whose role matches your StockIt role. The Status history table below lists every recorded change: when, from, to, payment tag, who and the note. Transitions are unrestricted by design — the history is the record, not a gate.
